...
class ControllerMarketingCoupon extends Controller {
	...
	protected function getForm() {
		...
		// ex-product
		if (isset($this->request->post['coupon_ex_product'])) {
			$products = $this->request->post['coupon_ex_product'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$products = $this->model_marketing_coupon->getCouponExProducts($this->request->get['coupon_id']);
		} else {
			$products = array();
		}
		...
		foreach ($products as $product_id) {
			...
			if ($product_info) {
				$data['coupon_ex_product'][] = array(
					'ex_product_id' => $product_info['product_id'],
					'ex_name'       => $product_info['name']
				);
			}
		}
		...
		//exclude category
		if (isset($this->request->post['coupon_ex_category'])) {
			$categories = $this->request->post['coupon_ex_category'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$categories = $this->model_marketing_coupon->getCouponExCategories($this->request->get['coupon_id']);
		} else {
			$categories = array();
		}
		...
		foreach ($categories as $category_id) {
			...
			if ($category_info) {
				$data['coupon_ex_category'][] = array(
					'ex_category_id' => $category_info['category_id'],
					'ex_name'        => ($category_info['path'] ? $category_info['path'] . ' &gt; ' : '') . $category_info['name']
				);
			}
		}
