...
class ControllerCatalogProduct extends Controller {
	...
	public function bulkadd(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if(!empty($this->request->files['file']['name']) && in_array($this->request->files['file']['type'], $csvMimes)){
				...
				if(is_uploaded_file($this->request->files['file']['tmp_name'])){
					...
					while(($line = fgetcsv($csvFile)) !== FALSE){
						...
						$this->model_catalog_product->addProductBulk($data);
			...
			$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'], true)); // . $url, true));
	...
	protected function getList() {
		...
		$data['bulkadd'] = $this->url->link('catalog/product/bulkadd', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$discount = round(($result['price']-$product_specials[0]['price'])*100/$result['price']);
			$new_mrp_update = (80/100)*$result['price'];
			if($result['price']<99999){

				$mrp_change = (15/100)*$result['price']+$result['price'];

			}else{

				$mrp_change = (20/100)*$result['price']+$result['price'];

			}


			$data['products'][] = array(
				'new_price'  => $new_mrp_update,
	...
	protected function getForm() {
		...
		if (isset($product_info['bestseller'])) {
			$data['bestseller'] = $product_info['bestseller'];
		}

	   // echo $product_info['fast_shipping'];exit;
		if (isset($product_info['fast_shipping'])) {
			$data['fast_shipping'] = $product_info['fast_shipping'];
		}
		...
		if($data['product_description']){
			...
			$ciid = (int)$data['product_description'][1]['main_category'];

			$main_category_name = $this->model_catalog_category->getCategory($ciid);

			//   print_r($main_category_name);exit;

			//$main_categoryss = $this->model_catalog_category->getCategory($ciid);

			$data['main_category_name'] = $main_category_name['name'];
...
if (isset($this->request->post['product_group'])) {
	$product_groups = $this->request->post['product_group'];
} elseif (isset($this->request->get['product_id'])) {
	$product_groups = $this->model_catalog_product->getProductGroups($this->request->get['product_id']);
} else {
...
foreach ($product_groups as $product_group) {
	$group_info = $this->model_catalog_group->getGroup($product_group['group_id']);

	if ($group_info) {
		$data['product_groups'][] = array(
			'group_id'                  => $product_group['group_id'],
			'text'          => $group_info['name'],
			'trail_product_id'          => $product_group['product_group_description']
...
// print_r($data['product_groups']);
		...
		if (isset($this->request->post['product_bundle'])) {
			...
			$products = $this->model_catalog_product->getProductBundle($this->request->get['product_id']);
		} else {
			$products = array();
		}
		
		$data['product_bundle'] = array();
		
		foreach ($products as $product_id) {
			$bundle_info = $this->model_catalog_product->getProduct($product_id);
	...
	protected function getFormBulk() {
		...
		$this->response->setOutput($this->load->view('catalog/product_form_bulk', $data));
	...
	public function updatePrice() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$res = $this->model_catalog_product->editPrice($this->request->post['pid'], $this->request->post['special_p']);
			if($res==1){
				return 1; 
			}else{ return 0;}
		}
	...
	public function updatePriceMrp() {
		$this->load->language('catalog/product');
		$this->load->model('catalog/product');

		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$res = $this->model_catalog_product->updatePriceMrp($this->request->post['pid'], $this->request->post['mrp']);
			if($res==1){
				return 1; 
			}else{ return 0;}
		}
