	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['orders'][] = array(
				...
				'stat_up'        => $this->url->link('sale/custom_order_status', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url, true),
	...
	public function info() {
		...
		if ($order_info) {
			...
			if($order_info['order_status'] == 'Pending'){
				$addressss = $order_info['payment_address_1'].' '.$order_info['payment_address_2'];
				$totalamt = round($order_info['total'], 0);
				  // print_r($order_info);exit;
				$paymentSting = "name=".$order_info['firstname']." ".$order_info['lastname']."#email=".$order_info['email']."#telephone=".$order_info['telephone']."#amount=".$totalamt."#order_id=".$order_info['order_id']."#address=".$addressss."#city=".$order_info['payment_city']."#state=".$order_info['payment_zone_code']."#country=".$order_info['payment_country']."#pincode=".$order_info['payment_postcode'];
				//echo $paymentSting;
				$newss = base64_encode($paymentSting);
				$data['payment_link'] = HTTPS_CATALOG.'custom-payment&ref='.$newss;
