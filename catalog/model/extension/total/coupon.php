...
class ModelExtensionTotalCoupon extends Model {
	...
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			...
			// ex product
			$coupon_ex_product_data = array();

			$coupon_ex_product_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_ex_product` WHERE coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "'");

			foreach ($coupon_ex_product_query->rows as $product) {
				$coupon_ex_product_data[] = $product['product_id'];
			}

			// ex category
			$coupon_ex_category_data = array();

			$coupon_ex_category_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_ex_category` cc LEFT JOIN `" . DB_PREFIX . "category_path` cp ON (cc.category_id = cp.path_id) WHERE cc.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "'");

			foreach ($coupon_ex_category_query->rows as $category) {
				$coupon_ex_category_data[] = $category['category_id'];
			}
			$product_ex_data = array();

			if ($coupon_ex_product_data || $coupon_ex_category_data) {
				foreach ($this->cart->getProducts() as $product) {
					if (in_array($product['product_id'], $coupon_ex_product_data)) {
						$product_ex_data[] = $product['product_id'];

						continue;
					}

					foreach ($coupon_ex_category_data as $category_id) {
						$coupon_ex_category_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "product_to_category` WHERE `product_id` = '" . (int)$product['product_id'] . "' AND category_id = '" . (int)$category_id . "'");

						if ($coupon_ex_category_query->row['total']) {
							$product_ex_data[] = $product['product_id'];
		...
		if ($status) {
			...
			return array(
				...
				'use_discount_tiers'    => $coupon_query->row['coupon_tiers']
	...
	public function getTotal($total) {
		...
		if (isset($this->session->data['coupon'])) {
			...
			if ($coupon_info) {
				...
				// Check if specific discount tiers are enabled
				if (!empty($coupon_info['use_discount_tiers']) && $coupon_info['use_discount_tiers'] == 1) {
					// Fetch discount tiers from a custom table
					$tiers_query = $this->db->query("SELECT * FROM `oc_coupon_tiers` WHERE `coupon_id` = '" . (int)$coupon_info['coupon_id'] . "' ORDER BY `min_cart_value` ASC");
	
					if ($tiers_query->num_rows) {
						$discount = 0;
	
						// Loop through the tiers to find the applicable discount
						foreach ($tiers_query->rows as $tier) {
							if ($sub_total >= (float)$tier['min_cart_value'] && $sub_total <= (float)$tier['max_cart_value']) {
								if ($tier['discount_type'] == 'percentage') {
									$discount = ($sub_total * $tier['discount']) / 100;
								} elseif ($tier['discount_type'] == 'fixed') {
									$discount = $tier['discount'];
								}
								break; // Apply the first matching tier
							}
						}
	
						// If a discount is calculated, apply it
						if ($discount > 0) {
							$total['totals'][] = array(
								'code'       => 'coupon',
								'title'      => 'Coupon ('. $coupon_info["name"] .')',
								'value'      => -$discount,
	...
	public function getAllCoupon(){
		$status = true;
		$coupon_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon` WHERE ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) AND status = '1' AND active = '1' ORDER BY `date_added` DESC");
