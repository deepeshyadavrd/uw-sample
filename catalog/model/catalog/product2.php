...
class ModelCatalogProduct2 extends Model {
	...
	public function getProductForCategory($product_id) {
		...
		echo ("SELECT DISTINCT p.product_id,p.model,p.sku ,p.quantity,p.price,p.date_available,p.length,p.width,p.height,p.minimum,p.status,p.viewed,p.date_added, pd.name AS name, pi.image as image2, pd.description AS description, p.image, m.name AS manufacturer, (
		SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) LEFT JOIN " . DB_PREFIX . "product_image pi ON p.product_id = pi.product_id AND pi.sort_order = 2 WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW()");
		if ($query->num_rows) {
			return array(
				'product_id'       => $query->row['product_id'],
