<?php
class ControllerApiNeworder extends Controller {
	public function add() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			// 	if (empty( $this->request->post['payment_methods'])) {
			...
			if (!$json) {
				...
				$order_data['payment_address_format'] = $this->request->post['address_format'];
				$order_data['payment_custom_field'] = (isset($this->session->data['payment_address']['custom_field']) ? $this->session->data['payment_address']['custom_field'] : array());

				if (isset( $this->request->post['payment_method']['title'])) {
					$order_data['payment_method'] =  $this->request->post['payment_method']['title'];
				} else {
					$order_data['payment_method'] = '';
				}

				if (isset( $this->request->post['payment_method']['code'])) {
					$order_data['payment_code'] =  $this->request->post['payment_method']['code'];
				} else {
				...
				foreach($products as $key => $product1){
					$product_details = $this->model_catalog_product2->getProduct($product1['product_id']);
					
						$products[$key]['name'] = $product_details['name'];
						$products[$key]['model'] = $product_details['model'];
						$products[$key]['subtract'] = $product1['quantity'];
						$products[$key]['price'] = $product1['unit_price'];
						$products[$key]['tax'] = 0;
						$products[$key]['reward'] = 0;
