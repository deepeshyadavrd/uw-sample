...
class ControllerProductRecentlyviewed extends Controller {
	public function index($setting) {
		...
		$data['heading_title'] = 'This is heading';//$this->language->get('heading_title');

		$data['text_tax'] = 'test tax';//$this->language->get('text_tax');

		$data['button_cart'] = 'button cart';//$this->language->get('button_cart');
		$data['button_wishlist'] = 'wishlist button';//$this->language->get('button_wishlist');
		$data['button_compare'] = 'button compare';//$this->language->get('button_compare');
		...
		if ($this->customer->isLogged()) {
			$this->load->model('catalog/recently_viewed');
			...
			if(isset($this->request->cookie['recently_viewed']) && !empty($this->request->cookie['recently_viewed'])) {
				...
				foreach($recently_viewed as $k=>$v){
					$this->model_catalog_recently_viewed->setRecentlyViewedProducts($this->customer->getId(), $k, $v);
				}
				unset($this->request->cookie['recently_viewed']);
				setcookie('recently_viewed', '', 0, '/', $this->request->server['HTTP_HOST']);
			}
			
			if($product_ids = $this->model_catalog_recently_viewed->getRecentlyViewedProducts($this->customer->getId(), $setting['limit'], $current_product_id)){
		...
		if ($results) {
			...
			return $this->load->view('product/recently_viewed', $data);
		}
	}
}