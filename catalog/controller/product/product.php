		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'], 746, 522),
					'img_thumb' => $this->model_tool_image->resize($result['image'], 96, 72)
				);
			}

			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				echo $data['price'] = $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			} else {
				$data['price'] = false;
			}

			if ((float)$product_info['special']) {
				echo $data['special'] = $this->currency->format($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			} else {
				$data['special'] = false;
			}
			...
			$data['rating5'] = ($product_info['reviews'] > 0)
    ? ($data['review5']/(int)$product_info['reviews'])*100: 0;
			$data['rating4'] = ($product_info['reviews'] > 0)
    ? ($data['review4']/(int)$product_info['reviews'])*100: 0;
			$data['rating3'] = ($product_info['reviews'] > 0)
    ? ($data['review3']/(int)$product_info['reviews'])*100: 0;
			$data['rating2'] = ($product_info['reviews'] > 0)
    ? ($data['review2']/(int)$product_info['reviews'])*100: 0;
			$data['rating1'] = ($product_info['reviews'] > 0)
    ? ($data['review1']/(int)$product_info['reviews'])*100: 0;
			...
			$data['buynow'] = $this->url->link("checkout/cart", "",true);
			$data['tags'] = array();
