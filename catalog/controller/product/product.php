...
// print_r($product_info);
		if ($product_info) {
			...
			list($name, $finish) = explode('(', $product_info['name']);
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'], 1920, 1340),
					'img_thumb' => $this->model_tool_image->resize($result['image'], 500, 500)
				);
			}
...
// echo $product_info['tax_class_id'];
			...
			$product_groups = $this->model_catalog_product2->getProductGroup($this->request->get['product_id']);
			...
			foreach($product_groups as $product_group){
				...
				foreach($product_group['group_product']  as $value){
					...
					if($value['p_image'] != NULL){
						if ($value['p_image']) {
							$image = $this->model_tool_image->resize($value['p_image'], 128, 88);
						} else {
...
// print_r($results);
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), 140);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height'));
				}
...
// print_r($special);
			...
			$results = $this->model_catalog_product2->getProductBundle($this->request->get['product_id']);
...
// print_r($special);
			...
			$data['recent_block'] = $this->load->controller('product/recentlyviewed', array(
				'limit' => 5,
				'image_width' => 80,
				'image_height' => 80
			));
			//print_r($data['recent_block']);
