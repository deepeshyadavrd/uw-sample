	...
	public function index() {
		if ($this->config->get('feed_google_base_status')) {
			...
			foreach ($google_base_categories as $google_base_category) {
				...
				$products = $this->model_catalog_product2->getProducts($filter_data);
				...
				foreach ($products as $product) {
					if (!in_array($product['product_id'], $product_data) && $product['description']) {
						...
						$output .= '<g:brand><![CDATA[Urbanwood]]></g:brand>';
						...
						$categories = $this->model_catalog_product2->getCategories($product['product_id']);
