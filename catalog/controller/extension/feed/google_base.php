	...
	public function index() {
		...
		if ($this->config->get('feed_google_base_status')) {
			...
			foreach ($google_base_categories as $google_base_category) {
				...
				$products = $this->model_catalog_product->getProductsForfeed($filter_data);
				...
				foreach ($products as $product) {
					...
					if (!in_array($product['product_id'], $product_data) && $product['description']) {
						...
						// $product_attribute = $this->model_catalog_product->getProductAttributes($product['product_id']);
						// foreach($product_attribute as $k => $v){
						// 	foreach($v['attribute'] as $key => $value){
						// 		if($value['name']=='Material'){
						// 			$output .= '  <g:material>' . $value['text'] . '</g:material>';
						// 		}
						// 		if($value['name']=='Finish'){
						// 			$output .= '  <g:finish>' . $value['text'] . '</g:finish>';
						// 		}
						// 		// echo '<p>'.$value['name'] ." - ". $value['text'].'</p>\n';
						// 	}
						// }
						...
						if ((float)$product['special']) {
							...
							$output .= '  <g:price>' .  round($this->currency->format($this->tax->calculate($product['special'], 0), $currency_code, $currency_value, false)) . '</g:price>';

						} else {

							$output .= '  <g:price>' . round($this->currency->format($this->tax->calculate($product['price'], 0), $currency_code, $currency_value, false)) . '</g:price>';

						}

						$output .= '<g:brand> Urbanwood </g:brand>';//'<g:brand><![CDATA[' . html_entity_decode($product['manufacturer'], ENT_QUOTES, 'UTF-8') . ']]></g:brand>';
						...
						if ($product['length']) {
							...
							$output .= '  <g:length>' . ($product['length'] + 0). '</g:length>';

						}
						if ($product['width']) {

							$output .= '  <g:width>' . ($product['width'] +0). '</g:width>';

						}
						if ($product['height']) {

							$output .= '  <g:height>' . ($product['height'] +0). '</g:height>';
