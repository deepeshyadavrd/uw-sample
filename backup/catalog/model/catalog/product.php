...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		$sql = "SELECT p.product_id, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
...
// 		if($data['filter_category_id'] != 70 AND $data['filter_category_id'] != 71 AND $data['filter_category_id'] != 72 AND $data['filter_category_id'] != 190 AND $data['filter_category_id'] != 90 AND $data['filter_category_id'] != 160 AND $data['sp'] != 'search_page'){
// 			$sql2 = "SET @row_number = -1;";
// 			$query = $this->db->query($sql2);
// 			$sql1 = "SELECT oc_product.product_id, rating, special FROM oc_product RIGHT JOIN ( SELECT product_id, rating, special FROM ( SELECT (@row_number:=@row_number+1) AS rownum, product_id, rating, special FROM ( ". $sql .") AS sorted ) as ranked WHERE rownum % 3 = 0 ) AS subset ON subset.product_id = oc_product.product_id";
	...
	public function getProductGroup($product_id) {
		...
		// if($product_attribute_query->rows[0]['name'] == 'Finish Option'){
			...
			foreach($product_query->rows as $value){
				$product_query = $this->db->query("SELECT pd.product_id,pd.name, ps.price, pi.image from " . DB_PREFIX . "product_description pd LEFT JOIN ". DB_PREFIX ."product p ON pd.product_id = p.product_id LEFT JOIN ". DB_PREFIX ."product_special ps ON p.product_id = ps.product_id LEFT JOIN ". DB_PREFIX ."product_image pi ON p.product_id = pi.product_id where p.status = 1 AND pi.sort_order = 1 AND p.product_id=". (int)$value['trail_product_id'] );
		// }else{
...
// print_r($product_group_data);
	...
	// 	$query = $this->db->query('SELECT * FROM oc_top_trending');
	// 	foreach ($query->rows as $result) {
	// 		$product_data[] = $result;
	// 	}
	// 	if ($product_data) {
